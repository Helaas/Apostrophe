...
- **Footer overflow chord disabled by default** (`apostrophe.h`): the L1+R1 shoulder button chord no longer opens the hidden-actions overlay. The Menu button is now the sole trigger for both the help overlay and footer overflow. A custom chord can be re-enabled via `ap_set_footer_overflow_opts()`.
