...
If your caller needs every successful standard-option change to return immediately, set `.return_on_option_change = 1`. Apostrophe will update `selected_option`, then return with `result.action == AP_ACTION_OPTION_CHANGED`, leaving `AP_ACTION_TRIGGERED` reserved for `action_button`.
